...
public class TaskManager_VR : MonoBehaviour
{
    // get the other scripts
    //public DisplayText displayText;
    public WriteCSV writeCSV;
    public ViveInput inputManager;
    ...
    string text2 = "Choose the chair you would rather sit in by pressing the Trigger on the corresponding Controller.";
    string text3 = "Press Touchpad to start.";
    ...
    void FixedUpdate()
    {
        ...
        {
            DisplayInstructions("Time for a pause. Press Touchpad to continue.", 30); // display pause screen
            breakCurr = false; // pause isi

            inputManager.hasStarted = false;
            inputManager.inTrial = false;
        ...
        if (startTrialButton[SteamVR_Input_Sources.Any].stateDown && !breakCurr && !end)
        {
            Debug.Log("Start");
            breakCurr = true;

            inputManager.hasStarted = true;
            inputManager.inTrial = true;
        ...
        if (breakCurr == true && stimCurr == false)
        {
            ...
            if (_breakInterval < 0.0)
            {
                ...
                inputManager.startTrialTime = Time.realtimeSinceStartup;
                writeCSV.cnt += 1;
                ...
                inputManager.objectSelected = false;
                inputManager.inTrial = true;
        ...
        if (stimCurr == true && breakCurr == false)
        {
            ...
            if (_stimInterval < 0.0)
            {
                ...
                inputManager.inTrial = false; //reset selection (color)
            ...
            else if (inputManager.objectSelected)
            {
                _breakInterval = breakInterval; // reset isi time
                breakCurr = true;
                stimCurr = false;
                ...
                DisplayInstructions("", 100);
                response = inputManager.objectHit; //get the selected object
                _rt = inputManager.rt; // reaction time
    ...
    {
       //if (currentTrial <= (nrTrials * nrBlocks))
       //{
            //Randomize Condition
